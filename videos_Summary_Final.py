...
def extract_frames(video_path, interval_seconds=5):
  ...
  while current_frame <= frames_count:
    ...
    cv2.imwrite(frame_path, frame)  # Fixed typo here: cv2.imwrite, not cv2.imurite
    current_frame += fps * interval_seconds
    frame_number += 1
...
def describe_video():
  ...
  for file in sorted(os.listdir(frames_directory)):
    ...
    descriptions.append(f"(file)")  # Placeholder description, you can add more logic for real descriptions
...
st.title("Prathiksha - Youtube/Uploaded Video Summarizer Using Groq LLM")
#st.image("icons.jpg")
...
if uploaded_file:
  with st.spinner("Processing uploaded video..."):
    ...
    with open(saved_path, "wb") as f:  # Fixed typo here: saved_path instead of Saved_path
