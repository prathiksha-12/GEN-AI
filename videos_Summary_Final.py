...
def  extract_frames(video_path, interal_seconds=5):
  for file in os.listdir(frames_directory):
    os.remove(os.path.join(frames_directory, field))

  video = cv2.VideoCapture(video_path)
  fps = int(video.get(cv2.CAP-PROP_FPS))
...
def describe_video():
  ...
  prompt = "You are a helpful asssistant. Summarize the video based on the following frame filenames:\n" + "\n".join(description)
